...
class ObjectDetection:
    def __init__(self):
        self.MODEL = cv2.dnn.readNet(
            'models/yolov3_training_final50000.weights',
            'models/yolov3_training.cfg'
        )
    ...
    def detectObj(self, snap):
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                print("Detected label:", label)  # Add this line for debugging

                if label == "merokok":
                    # Simpan hasil deteksi ke dalam database
                    timestamp = datetime.datetime.now()
                    result = "Terdeteksi Merokok"
                    new_detection = Detection(timestamp=timestamp, result=result)
                    session.add(new_detection)
                    session.commit()
                    # Play alarm sound
                    winsound.PlaySound("alert3.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)  # Remove SND_LOOP for debugging
                else:
                    # Stop alarm sound
    ...
    def show(self):
        while(self.VIDEO.isOpened()):
            ...
            if ret == True:
                ...
                break
        # Stop alarm sound when the video stream ends
        winsound.PlaySound(None, winsound.SND_PURGE)
        print('off')
